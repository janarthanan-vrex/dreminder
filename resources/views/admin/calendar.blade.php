...
<style>
  ...
  .cal-cell-v2.cal-today-v2 {
    background: linear-gradient(135deg, rgba(124, 58, 237, .22), rgba(13, 148, 136, .12));
  ...
  .cal-overlay.open .cal-modal-box {
      ...
      <div class="qc-body">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
          ...
          <div>
            ...
            <select class="qc-input" id="qc-sub" disabled>
...
<script>
    ...
    const USER_CALENDAR_URL =
        "{{ route('admin.calendar.user.data') }}";
...
<script>
  ...
  //       {
  //         id: 'r020',
  //         title: 'Car Pollution Check',
  //         category: 'motor-vehicle',
  //         subcategory: 'PUC Certificate',
  //         dueDate: '2026-04-28',
  //         dueTime: '',
  //         status: 'active',
  //         provider: 'RTO Authorised',
  //         cost: '200'
  //       },
  //       {
  //         id: 'r021',
  ...
  //     reminders: [{
  //         id: 'r028',
  //         title: 'Kids School Fees',
  //         category: 'home',
  //         subcategory: 'Education',
  //         dueDate: '2026-04-30',
  //         dueTime: '',
  //         status: 'active',
  //         provider: 'St. Thomas School',
  //         cost: '35000',
  //         frequency: 'Quarterly'
  //       },
  //       {
  ...
  //     id: 'u005',
  //     name: 'Rahul Pillai',
  //     email: 'rahul.pillai@hotmail.com',
  //     plan: 'Free',
  //     avatar: 'RP',
  //     avatarBg: '#f43f5e',
  //     reminders: [{
  ...
  //         dueTime: '09:00',
  //         status: 'active',
  //         provider: 'TVS Service',
  //         cost: '1200'
  //       },
  //       {
  //         id: 'r037',
  //         title: 'BSNL Landline Bill',
  //         category: 'tv-telephone-mobile',
  ...
  //         dueTime: '08:00',
  //         status: 'active',
  //         provider: 'Local Painter',
  //         cost: '25000'
  //       },
  //       {
  //         id: 'r043',
  //         title: "Mother-in-Law Birthday",
  //         category: 'special-days',
  //         subcategory: 'Birthday',
  //         dueDate: '2026-05-06',
  //         dueTime: '',
  ...
  //       {
  //         id: 'r044',
  //         title: 'Two-Wheeler Tax',
  //         category: 'motor-vehicle',
  //         subcategory: 'Road Tax',
  //         dueDate: '2026-05-09',
  //         dueTime: '',
  //         status: 'active',
  //         provider: 'RTO',
  ...
  let _calY, _calM, _calCatFilter = 'all',
    _selDay = null,
    _selUserId = null;
  window._selDay = null;
    ...
    dd.innerHTML = filtered.length === 0
        ...
        : filtered.map(u => `
            <div class="user-drop-item"
                 onclick="adminCalSelectUser('${u.id}')">

                <div class="user-drop-avatar">
 ...
 async function adminCalSelectUser(userId)
{
    ...
    try {
        ...
        const res = await fetch(
            `${USER_CALENDAR_URL}?user_id=${userId}`
        );
        ...
        if (data.status) {
            ...
            ALL_REMINDERS = data.histories;

            _renderAll();

            toast(
                `Now viewing ${u.name}'s calendar`,
                'success'
            );
        }
 ...
 function adminCalClearUser()
{
    ...
    document.getElementById('selected-user-pill')
        .style.display = 'none';

    document.getElementById('cal-user-hint-text')
        .textContent = 'Select a user to view their reminders';
 ...
 function _buildCatFilter()
{
    ...
    Object.entries(CAL_CATS).forEach(([k, c]) => {
        ...
        btn.dataset.cat = String(k);

        btn.onclick = () =>
            calSetCatFilter(String(k), btn);

        btn.innerHTML = `
            <div class="cat-legend-dot"
  ...
  if (_calCatFilter !== 'all')
{
  ...
  function _renderGrid() {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      cell.onclick = (e) => {
        if (!e.target.classList.contains('cal-add-btn') && !e.target.closest('.cal-add-btn')) _selectDay(ds, d);
      };
      ...
      let chipsHTML = visible.map(r => {
        const cat = CAL_CATS[String(r.category)] || {
          color: '#94a3b8',
 ...
 function qcUpdateSubs()
{
    ...
    const sub =
        document.getElementById('qc-sub');
 ...
 function calExport()
{
    ...
    const u = USERS.find(u => u.id == _selUserId);

    toast(
        `Exported ${u.name}'s calendar`,
        'success'
    );
}
