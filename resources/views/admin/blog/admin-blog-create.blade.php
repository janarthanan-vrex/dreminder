...
<div class="blog-edit-grid">
    ...
    <div class="blog-sidebar">
        ...
        <div class="section-card" style="margin:0">
            <div class="section-title" style="margin-bottom:14px"><i class="ri-send-plane-line" style="color:var(--purple)"></i> Publish</div>

            <!-- Status toggle -->
            <div class="status-row" style="margin-bottom:14px">
                <label class="toggle-switch"><input type="checkbox" name="is_active" id="status-toggle" checked onchange="syncStatusLabel()"><span class="toggle-slider"></span></label>
                <div>
                    <div style="font-size:.85rem;font-weight:700;color:var(--text)" id="status-label-text">Active</div>
                    <div style="font-size:.72rem;color:var(--text4)" id="status-sub-text">Post is visible to the public</div>
                </div>
            </div>
...
function submitPost() {
    ...
    form.append('category',         document.querySelector('[name="category"]').value);
    ...
    fetch('{{ route("admin.blog.store") }}', { method: 'POST',headers: { 'Accept': 'application/json' }, body: form })
        ...
        .then(({ status, data }) => {
            if (status === 422) {
                ...
                setTimeout(() => window.location.href = '{{ route("admin.blog.index") }}', 1000);
            }
        })
        .catch(() => { if (typeof toast === 'function') toast('Something went wrong.', 'error'); });
}
</script>
...
function toggleSourceMode(){
    ...
    if(!sourceMode){
        ...
        body.style.display='none'; src.style.display='block';
        btn.classList.add('active'); sourceMode=true;
    } else {
        body.innerHTML = src.value;
        src.style.display='none'; body.style.display='block';
        btn.classList.remove('active'); sourceMode=false;
    }
}

function insertLink(){ var url=prompt('Enter URL:'); if(url) document.execCommand('createLink',false,url); }
function insertImage(){ var url=prompt('Enter image URL:'); if(url) document.execCommand('insertImage',false,url); }
...
function toggleAutoSlug(){
    var slugInput = document.getElementById('post-slug');
    var autoSlugInput = document.getElementById('auto-slug');
    autoSlug = autoSlugInput ? autoSlugInput.checked : autoSlug;
    if(slugInput) {
...
function saveInactive(){
    ...
    syncStatusLabel();
    prepareSubmit();
    if(typeof toast==='function') toast('Saved as inactive','success');
}
