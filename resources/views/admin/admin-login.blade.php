...
.input-icon-right{position:absolute;right:14px;top:50%;transform:translateY(-50%);color:rgba(255,255,255,.25);font-size:.9rem;cursor:pointer;transition:color .2s;background:none;border:none;padding:0;line-height:1}
.input-icon-right:hover{color:rgba(255,255,255,.7)}
.form-err{font-size:.72rem;color:#f87171;margin-top:5px;display:none;align-items:center;gap:4px}
.form-err.show{display:flex}
.step-dot{width:8px;height:8px;border-radius:50%;display:inline-block}
.divider{display:flex;align-items:center;gap:12px;margin:20px 0}
.divider span{font-size:.72rem;color:rgba(255,255,255,.25);white-space:nowrap;font-weight:500;letter-spacing:.04em;text-transform:uppercase}
.divider::before,.divider::after{content:'';flex:1;height:1px;background:rgba(255,255,255,.08)}
.remember-wrap{display:flex;align-items:center;gap:8px;cursor:pointer}
.remember-wrap input[type=checkbox]{width:16px;height:16px;border-radius:4px;accent-color:#7c3aed;cursor:pointer;flex-shrink:0}
.remember-wrap span{font-size:.8rem;color:rgba(255,255,255,.45);user-select:none}
.security-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);font-size:.67rem;font-weight:600;color:#34d399;letter-spacing:.03em}
.login-alert{margin-top:14px;padding:10px 14px;border-radius:10px;background:rgba(244,63,94,.1);border:1px solid rgba(244,63,94,.25);font-size:.78rem;color:#fda4af;display:none;align-items:center;gap:8px}
...
<div style="width:100%;max-width:460px;position:relative;z-index:10" class="fade-up">
  ...
  <div class="glass-strong" style="border-radius:24px;padding:32px" id="loginCard">
    ...
    <div style="margin-bottom:18px">
      <label class="auth-label" for="loginEmail">Admin Email Address</label>
      <div class="input-wrap">
        <i class="input-icon ri-mail-line"></i>
        <input type="email" autocomplete="off" id="loginEmail" class="auth-input" placeholder="admin@dremind.co.uk" autocomplete="email">
      </div>
      <div class="form-err" id="emailErr"><i class="ri-error-warning-line"></i>&nbsp;Please enter a valid email address.</div>
    </div>
    ...
    <div style="margin-bottom:20px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        ...
        <a href="admin-forgot-password" style="font-size:.73rem;color:#a78bfa;text-decoration:none;font-weight:500;transition:color .2s" onmouseover="this.style.color='#c4b5fd'" onmouseout="this.style.color='#a78bfa'">Forgot password?</a>
      </div>
      <div class="input-wrap">
      ...
      <div class="form-err" id="pwdErr"><i class="ri-error-warning-line"></i>&nbsp;Password must be at least 8 characters.</div>
    </div>
    ...
    <div class="login-alert" id="loginAlert">
      <i class="ri-close-circle-line" style="font-size:1rem;color:#f43f5e;flex-shrink:0"></i>
...
function toast(msg,type){
  ...
  setTimeout(function(){t.style.opacity='0';t.style.transform='translateY(8px)';setTimeout(function(){t.remove();},300);},3000);
}
...
function attemptLogin(){
  ...
  if(!email||!email.includes('@')||!email.includes('.')){
    document.getElementById('emailErr').classList.add('show');valid=false;
  }
  if(pwd.length<8){
    document.getElementById('pwdErr').classList.add('show');valid=false;
  }
  if(!valid){
  ...
  setTimeout(function(){
    ...
    toast('Welcome back, Admin! Redirecting&hellip;','success');
    setTimeout(function(){
...
document.addEventListener('keydown',function(e){if(e.key==='Enter')attemptLogin();});

/* Particles — identical hue range to forgot-password page */
(function(){
